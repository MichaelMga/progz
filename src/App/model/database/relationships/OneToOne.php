...
class OneToOne implements RelationshipInterface
{
    ...
    public function triggerConstraintBuilding() : void
    {
        ...
        $firstEntityUniqueConstraint = new UniqueConstraint($this->secondEntity . "_id");
        $this->firstEntityTable->getColumnHandler()->addConstraint($firstEntityUniqueConstraint);

        $firstEntityFkConstraint = new ForeignkeyConstraint($this->secondEntity);
        $this->firstEntityTable->getColumnHandler()->addConstraint($firstEntityFkConstraint);

        //second entity

        $secondEntityUniqueConstraint = new UniqueConstraint($this->firstEntity . "_id");
        $this->firstEntityTable->getColumnHandler()->addConstraint($secondEntityUniqueConstraint);

        $secondEntityFkConstraint = new ForeignkeyConstraint($this->firstEntity);
        $this->firstEntityTable->getColumnHandler()->addConstraint($secondEntityFkConstraint);
    ...
    {
        $this->firstEntityTable->getColumnHandler()->addColumn($this->secondEntity . "_id","INT");
